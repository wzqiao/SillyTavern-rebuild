...
async function importWorldbookFromFile(event: Event): Promise<void> {
    ...
    try {
        ...
        notice.value = {
            tone: 'danger',
            message: `Worldbook import failed: ${describeError(error)}`,
        };
    } finally {
...
function handleImportResult(result: ReforgedWorldbookImportResult): void {
    ...
    const warningText = result.warnings.length ? ` ${result.warnings.length} warning(s) need review.` : '';
    notice.value = {
        tone: 'success',
        message: `${result.worldbook.name} imported with ${result.worldbook.entries.length} entries.${warningText}`,
    };
}
...
function selectWorldbook(item: ReforgedWorldbookLibraryItem): void {
    ...
    notice.value = {
        tone: 'neutral',
        message: `${item.worldbook.name} is now the active worldbook.`,
    };
}
...
function removeWorldbook(item: ReforgedWorldbookLibraryItem): void {
    ...
    notice.value = {
        tone: 'neutral',
        message: `${item.worldbook.name} removed from the library.`,
    };
}
...
function clearWorldbooks(): void {
    ...
    notice.value = {
        tone: 'neutral',
        message: 'Worldbook library cleared.',
    };
}
...
function summarizeRoutedPreview(lorebook: ReforgedChatLorebookContext | null): string {
    if (!lorebook) {
        return 'No active worldbook selected.';
    }
    ...
    const parts = [
        `${lorebook.beforeEntries?.length ?? 0} before`,
        `${lorebook.afterEntries?.length ?? 0} after`,
        `${(lorebook.authorNoteBeforeEntries?.length ?? 0) + (lorebook.authorNoteAfterEntries?.length ?? 0)} author note`,
        `${lorebook.exampleEntries?.length ?? 0} example`,
        `${depthCount} depth`,
        `${outletCount} outlet`,
    ];
...
function describeEntryTitle(entry: ReforgedWorldbookEntry): string {
    return entry.comment.trim() || entry.primaryKeys.join(', ') || `Entry ${entry.id}`;
}

function describePreviewTitle(entry: ReforgedChatLorebookEntryContext): string {
    return entry.title?.trim() || `Entry ${entry.id}`;
}
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="mt-3 text-3xl font-black tracking-[-0.03em] text-white sm:text-4xl">
                        Lore library and prompt preview
                    </h1>
                    <p class="mt-3 max-w-3xl text-sm leading-7 text-neutral-300">
                        Import SillyTavern world info JSON, choose the active book, and inspect the entries that can feed the chat prompt.
                    </p>
                </div>
        ...
        <div class="grid gap-5 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.35fr)]">
            <aside class="flex min-w-0 flex-col gap-5">
                <section class="rounded-[1.75rem] border border-white/10 bg-neutral-900/88 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.28)] sm:p-5">
                    ...
                    >
                        ...
                        <span class="text-sm font-bold">
                            {{ importBusy ? 'Reading worldbook...' : 'Choose JSON file' }}
                        </span>
                        <span class="max-w-sm text-xs leading-5 text-cyan-100/72">
                            Supports SillyTavern world info and Character Book JSON files.
                        </span>
                    </button>
                ...
                <section class="rounded-[1.75rem] border border-white/10 bg-neutral-900/88 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.28)] sm:p-5">
                    ...
                    >
                        No worldbooks imported yet. Choose a JSON file to build the library.
                    </div>
                </section>
            </aside>
                ...
                <section class="rounded-[1.75rem] border border-white/10 bg-neutral-900/88 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.28)] sm:p-5">
                    ...
                    >
                        ...
                        <div class="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                                ...
                                <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm leading-6 text-neutral-400">
                                            Primary keys identify entries that can be activated by scan text.
                                        </p>
                                    </div>
                                    <span class="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-bold text-neutral-200">
                                        {{ worldbookSummary.entriesWithPrimaryKeys }} keyed
                                    </span>
                                </div>
                            ...
                            <section class="min-w-0 rounded-[1.5rem] border border-white/10 bg-black/18 p-4">
                                ...
                                <p class="mt-1 text-sm leading-6 text-neutral-400">
                                    Type likely chat text to see entries that may participate in the prompt.
                                </p>
                                <Textarea
                                    v-model="previewScanText"
                                    class="mt-4"
                                    label="Preview scan text"
                                    placeholder="Mention a primary key, location, faction, or topic..."
                                    :rows="4"
                                />
                                <div class="mt-3 rounded-2xl border border-cyan-300/18 bg-cyan-300/8 px-4 py-3">
                                    <p class="text-xs font-black uppercase tracking-[0.18em] text-cyan-100">
                                        {{ activePreviewCount }} active preview entries
                                    </p>
                        ...
                        <section class="rounded-[1.5rem] border border-white/10 bg-black/18 p-4">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm leading-6 text-neutral-400">
                                        Active preview uses the existing lore context service; entry samples below are enabled entries from the selected JSON.
                                    </p>
                                </div>
                                <span class="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-bold text-neutral-200">
                                    Showing {{ Math.min(enabledEntrySamples.length, ENTRY_SAMPLE_LIMIT) }}
                                </span>
                            </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p class="mt-2 line-clamp-3 text-sm leading-6 text-emerald-50/78">
                                        {{ entry.content || 'No prompt content.' }}
                                    </p>
                                </article>
                            </div>
                            ...
                            >
                                No entries are active for the current preview text. Constant entries still appear automatically when present.
                            </div>

                            <Collapse
                                class="mt-4"
                                title="Enabled entry samples"
                                description="A compact look at the source entries without turning this page into a full editor."
                            >
                                <div
                                ...
                                >
                                    This worldbook has no enabled entries.
                                </p>
                            </Collapse>
                        </section>
                    </div>
                    ...
                    >
                        <p class="text-lg font-black text-white">
                            Select or import a worldbook
                        </p>
                        <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-neutral-400">
                            The detail pane will show entry counts, key coverage, warnings, and a lightweight prompt preview.
                        </p>
                    </div>
                </section>
            </main>
