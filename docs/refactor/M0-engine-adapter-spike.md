...
- Typed connection runtime handoff in `app/src/contracts/connection.ts` / `app/src/stores/connectionStore.ts`, distinguishing empty, incomplete, complete-unapplied, applied-but-unwired, and ready-to-attempt states. HomeView now consumes the handoff for Runtime gating and provider-to-generation API mapping instead of hard-coding `api: 'openai'` in the send path. This is still memory-only and does not persist secrets, test connectivity, or write SillyTavern runtime settings.
- Runtime connection guard in `app/src/contracts/connection.ts` / `app/src/stores/connectionStore.ts`, separating "runtime adapter diagnostics passed" from "the applied connection draft is injected into real requests". Until a reviewed injection seam exists, Runtime mode remains `applied-but-unwired` instead of pretending the memory draft can drive `sendOpenAIRequest`.
...
- Decide how the typed memory-only connection handoff should be safely injected into the SillyTavern runtime adapter or backend settings layer; current investigation found no safe app-side per-request `baseUrl/model/apiKey` option on `sendOpenAIRequest`.
- After an injection seam is reviewed, trigger one real OpenAI-compatible generation with user-provided API settings and confirm whether streaming data can be consumed through the `sendOpenAIRequest` path.
